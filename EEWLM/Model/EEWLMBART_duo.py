...
# 模型路径配置（请根据实际情况修改）
BART_MODEL_PATH = r"D:\zjn\three\model\bart-large"  # 本地BART模型路径
...
class EarthquakeEarlyWarningLLM(nn.Module):
    ...
    def __init__(
            ...
            output_head_class1: nn.Module = MagnitudeHead,
            output_head_class2: nn.Module = MagnitudeHead,
            output_head_class3: nn.Module = MagnitudeHead,
            output_activation1: nn.Module = nn.Tanh,
            output_activation2: nn.Module = nn.Tanh,
            output_activation3: nn.Module = nn.Tanh,
        ...
        # ==================== 输出头 ====================
        self.output_head1 = output_head_class1(
            feature_channels=reshape_out_dim,
            out_activation=output_activation1,
        )
        self.output_head2 = output_head_class2(
            feature_channels=reshape_out_dim,
            out_activation=output_activation2,
        )
        self.output_head3 = output_head_class3(
            feature_channels=reshape_out_dim,
            out_activation=output_activation3,
        )
    ...
    def forward(
        ...
        # 任务头
        output1 = self.output_head1(recovered, waveform)
        output2 = self.output_head2(recovered, waveform)
        output3 = self.output_head3(recovered, waveform)
...
# 模型工厂函数
# ============================================================================
def EEW_LMBART() -> EarthquakeEarlyWarningLLM:

    return EarthquakeEarlyWarningLLM(
        output_head_class1=AzimuthHead,
        output_head_class2=MagnitudeHead,
        output_head_class3=DistanceHead,
        reshape_out_dim=256,
        output_activation1=nn.Tanh,
        output_activation2=None,
        output_activation3=nn.Tanh,
...
#     # 测试震级估计模型
#     print("\n测试震级估计模型...")
#     model_mag =EEW_LMBART().to(device)
#     with torch.no_grad():
...
#     # # 测试震中距估计模型
#     # print("\n测试震中距估计模型...")
#     # model_dis = EEW_LMBART_dis().to(device)
#     # with torch.no_grad():
...
#     # # 测试方位角估计模型
#     # print("\n测试方位角估计模型...")
#     # model_azi = EEW_LMBART_azi().to(device)
#     # with torch.no_grad():
