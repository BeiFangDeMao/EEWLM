...
from transformers import BertModel, BertConfig, __version__ as transformers_version
...
# 模型路径配置（请根据实际情况修改）
BERT_MODEL_PATH = r"G:\迁移学习\LLM\Models\bert-large-uncased"  # 本地BERT模型路径
...
def generate_bert_target_modules(target_layers: List[int]) -> List[str]:
    """
    为 BERT 模型的指定层生成 LoRA 目标模块名称。
    ...
    for layer_idx in target_layers:
        ...
        target_modules.extend([
            f"layer.{layer_idx}.attention.self.query",
            f"layer.{layer_idx}.attention.self.key",
            f"layer.{layer_idx}.attention.self.value",
            f"layer.{layer_idx}.attention.output.dense",
        ])
        # 前馈网络层模块
        target_modules.extend([
            f"layer.{layer_idx}.intermediate.dense",
            f"layer.{layer_idx}.output.dense",
        ])
...
class BERTEncoderBlock(nn.Module):
    """
    BERT 编码器模块，支持 LoRA 微调和层截取。

    该模块加载预训练的 BERT 模型，可选择性地截取部分层，
    ...
    def __init__(
        ...
        # 加载预训练 BERT 模型
        if use_pretrained:
            print(f"正在加载本地 BERT 模型：{model_path}")
            ...
            warnings.warn("使用随机初始化的 BERT Encoder，建议优先使用预训练模型！")
            self.bert = BertModel(BertConfig())
            self.encoder = self.bert.encoder

        # 截取指定范围的编码器层
        self.encoder.layer = self.encoder.layer[start_layer:end_layer]
        print(f"已截取 BERT Encoder layers：{start_layer} ~ {end_layer - 1}（共 {end_layer - start_layer} 层）")
    ...
    def _validate_lora_target_modules(self) -> None:
        """验证 LoRA 目标模块是否存在于 BERT Encoder 中"""
    ...
    def _freeze_base_parameters(self) -> None:
        ...
        for name, param in self.encoder.named_parameters():
            ...
            # 解冻 LoRA、层归一化、位置嵌入
            elif any(keyword in name for keyword in ["lora", "LayerNorm", "layer_norm", "position_embeddings"]):
...
class EarthquakeEarlyWarningLLM(nn.Module):
    ...
    def __init__(
        ...
        target_modules = generate_bert_target_modules(target_layers)
        ...
        self.bert_encoder = BERTEncoderBlock(
            start_layer=0,
