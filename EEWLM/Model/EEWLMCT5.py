...
本模块实现了基于Chronos_T5模型的多模态地震数据处理框架，包含：
1. 多尺度卷积输入头（处理波形、频谱、特征数据）
2. Chronos_T5大语言模型主干（支持LoRA微调）
...
from transformers import T5Model, T5Config, __version__ as transformers_version
...
# 模型路径配置（请根据实际情况修改）
CHRONOS_T5_PATH = r"G:\迁移学习\LLM\Models\chronos-T5-large"  # 修改为你的实际路径
...
# ============================================================================
# 第二部分：Chronos_T5 大语言模型模块 (支持 LoRA 微调)
# ============================================================================
...
def generate_t5_target_modules(target_layers: List[int]) -> List[str]:
    """
    为 T5 模型的指定层生成 LoRA 目标模块名称。
    ...
    for layer_idx in target_layers:
        ...
        target_modules.extend([
            f"block.{layer_idx}.layer.0.SelfAttention.q",
            f"block.{layer_idx}.layer.0.SelfAttention.k",
            f"block.{layer_idx}.layer.0.SelfAttention.v",
            f"block.{layer_idx}.layer.0.SelfAttention.o"
        ])
        # 前馈网络层模块
        target_modules.extend([
            f"block.{layer_idx}.layer.1.DenseReluDense.wi",
            f"block.{layer_idx}.layer.1.DenseReluDense.wo"
        ])
...
class T5EncoderBlock(nn.Module):
    """
    T5 编码器模块，支持 LoRA 微调和层截取。

    该模块加载预训练的 T5 编码器，可选择性地截取部分层，
    ...
    def __init__(
            ...
            model_path: str = CHRONOS_T5_PATH
    ):
        """
        Args:
        ...
        # 加载预训练 T5 模型（仅编码器部分）
        if use_pretrained:
            print(f"正在加载本地 T5 模型：{model_path}")
            self.t5_encoder = T5Model.from_pretrained(
                pretrained_model_name_or_path=model_path,
                local_files_only=True,
            ).encoder
        else:
            warnings.warn("使用随机初始化的 T5 Encoder，建议优先使用预训练模型！")
            self.t5_encoder = T5Model(T5Config()).encoder

        # 截取指定范围的编码器层
        self.t5_encoder.block = self.t5_encoder.block[start_layer:end_layer]
        print(f"已截取 T5 Encoder blocks：{start_layer} ~ {end_layer - 1}（共 {end_layer - start_layer} 层）")
        ...
        if freeze_base and use_pretrained and lora_config is not None:
            ...
            # 非冻结模式下仍需冻结词嵌入层
            self._freeze_token_embeddings()

    def _validate_lora_target_modules(self) -> None:
        """验证 LoRA 目标模块是否存在于 T5 Encoder 中"""
        model_module_names = set()
        for name, _ in self.t5_encoder.named_parameters():
    ...
    def _freeze_token_embeddings(self) -> None:
        """冻结词嵌入层参数"""
        for name, param in self.t5_encoder.named_parameters():
            if "embed_tokens" in name:
                param.requires_grad = False
...
class EarthquakeEarlyWarningLLM(nn.Module):
    ...
    def __init__(
        ...
        target_modules = generate_t5_target_modules(target_layers)
        ...
        self.t5_encoder = T5EncoderBlock(
            start_layer=0,
    ...
    def forward(
        ...
        # T5 编码
        encoded = self.t5_encoder(tokens)
...
def create_eew_model_azimuth() -> EarthquakeEarlyWarningLLM:
    """创建方位角估计模型"""
...
def create_eew_model_magnitude() -> EarthquakeEarlyWarningLLM:
    """创建震级估计模型"""
    return EarthquakeEarlyWarningLLM(
...
def create_eew_model_distance() -> EarthquakeEarlyWarningLLM:
    """创建震中距估计模型"""
    return EarthquakeEarlyWarningLLM(
...
def create_eew_model_pphase() -> EarthquakeEarlyWarningLLM:
    """创建 P 波拾取模型"""
    return EarthquakeEarlyWarningLLM(
        llm_layers=4,
...
#     # 测试 P 波拾取模型
#     print("\n测试 P 波拾取模型...")
#     model = create_eew_model_pphase().to(device)
#     print(model)
#
#     model.eval()
#     print("\n模型初始化成功")
...
#         print(f"P波拾取模型输出形状: {output.shape}")
#
#     # 测试震级估计模型
#     print("\n测试震级估计模型...")
#     model_mag = create_eew_model_magnitude().to(device)
#     with torch.no_grad():
