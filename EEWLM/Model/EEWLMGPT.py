...
本模块实现了基于GPT2模型的多模态地震数据处理框架，包含：
1. 多尺度卷积输入头（处理波形、频谱、特征数据）
2. GPT2大语言模型主干（支持LoRA微调）
...
from transformers import GPT2Model, GPT2Config, __version__ as transformers_version
...
# 模型路径配置（请根据实际情况修改）
GPT2_MODEL_PATH = r"G:\迁移学习\LLM\Models\gpt2-medium"  # 修改为你的实际路径
...
def configure_lora(
    ...
    return LoraConfig(
        ...
        bias=bias,
        task_type=task_type,
        fan_in_fan_out=True,  # GPT2 需要使用这个参数
    )


def generate_gpt2_target_modules(target_layers: List[int]) -> List[str]:
    """
    为 GPT2 模型的指定层生成 LoRA 目标模块名称。
    ...
    for layer_idx in target_layers:
        ...
        target_modules.extend([
            f"h.{layer_idx}.attn.c_attn",  # 查询、键、值投影的合并矩阵
            f"h.{layer_idx}.attn.c_proj",  # 输出投影
        ])
        # 前馈网络层模块
        target_modules.extend([
            f"h.{layer_idx}.mlp.c_fc",  # 第一层线性变换
            f"h.{layer_idx}.mlp.c_proj",  # 第二层线性变换
        ])
...
class GPT2EncoderBlock(nn.Module):
    """
    GPT2 编码器模块，支持 LoRA 微调和层截取。

    该模块加载预训练的 GPT2 模型，可选择性地截取部分层，
    ...
    def __init__(
            ...
            model_path: str = GPT2_MODEL_PATH
    ):
        """
        Args:
        ...
        # 加载预训练 GPT2 模型
        if use_pretrained:
            print(f"正在加载本地 GPT2 模型：{model_path}")
            self.gpt2 = GPT2Model.from_pretrained(
                pretrained_model_name_or_path=model_path,
                local_files_only=True,
            )
        else:
            warnings.warn("使用随机初始化的 GPT2 Model，建议优先使用预训练模型！")
            self.gpt2 = GPT2Model(GPT2Config())

        # 截取指定范围的 Transformer 层
        self.gpt2.h = self.gpt2.h[start_layer:end_layer]
        print(f"已截取 GPT2 blocks：{start_layer} ~ {end_layer - 1}（共 {end_layer - start_layer} 层）")
    ...
    def _validate_lora_target_modules(self) -> None:
        """验证 LoRA 目标模块是否存在于 GPT2 模型中"""
        model_module_names = set()
        for name, _ in self.gpt2.named_parameters():
            # 移除权重/偏置后缀
    ...
    def _freeze_base_parameters(self) -> None:
        ...
        for name, param in self.gpt2.named_parameters():
            ...
            elif any(keyword in name for keyword in ["lora", "ln_", "wpe"]):  # LoRA、层归一化、位置嵌入
...
class EarthquakeEarlyWarningLLM(nn.Module):
    ...
    def __init__(
        ...
        target_modules = generate_gpt2_target_modules(target_layers)
        ...
        self.gpt2_encoder = GPT2EncoderBlock(
            start_layer=0,
    ...
    def forward(
        ...
        # GPT2 编码
        encoded = self.gpt2_encoder(tokens)
