...
本模块实现了基于T5模型的多模态地震数据处理框架，包含：
1. 多尺度卷积输入头（处理波形、频谱、特征数据）
2. T5大语言模型主干（支持LoRA微调）
...
# 模型路径配置（请根据实际情况修改）
T5_MODEL_PATH = r"G:\迁移学习\LLM\Models\t5-large"  # 修改为你的实际路径
...
class T5EncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        if use_pretrained:
            ...
            self.t5 = T5Model(T5Config())
            self.encoder = self.t5.encoder

        # 截取指定范围的编码器层
        self.encoder.block = self.encoder.block[start_layer:end_layer]
    ...
    def _freeze_base_parameters(self) -> None:
        ...
        for name, param in self.encoder.named_parameters():
            ...
            # 解冻 LoRA、层归一化、位置嵌入
            elif any(keyword in name for keyword in ["lora", "layer_norm", "embed_positions"]):
...
#     # 测试 P 波拾取模型
#     print("\n测试 P 波拾取模型...")
#     model_pp = create_eew_model_pphase().to(device)
#     print(model_pp)
#
#     model_pp.eval()
#     print("\nP波拾取模型初始化成功")
...
#     # 测试震中距估计模型
#     print("\n测试震中距估计模型...")
#     model_dis = create_eew_model_distance().to(device)
#     with torch.no_grad():
...
#     # 测试方位角估计模型
#     print("\n测试方位角估计模型...")
#     model_azi = create_eew_model_azimuth().to(device)
#     with torch.no_grad():
