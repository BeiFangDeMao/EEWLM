...
# -*- coding: utf-8 -*-
# @Project : LM
# @File : train4zuiyou.py
# @IDE : PyCharm
# @Author : 张嘉南
# @Date : 2025/10/31 下午9:37
"""
Chronos-T5 + LoRA 地震方位角(BAZ)估计模型训练脚本
...
MODEL_REGISTRY = {
    "EEW_LMChronos_t5_azi": EEW_LMChronos_t5_azi,
    "EEW_LM_t5_azi": EEW_LM_t5_azi,
    "EEW_LM_GPT2_azi": EEW_LM_GPT2_azi,
    "EEW_LMBERT_azi": EEW_LMBERT_azi,
    "EEW_LMBART_azi": EEW_LMBART_azi,
    "EEW_LMBART_aziA":EEW_LMBART_aziA,
    "EEW_LMBART_aziB":EEW_LMBART_aziB,
    "EEW_LMBART_aziC":EEW_LMBART_aziC
...
CHECKPOINT_DIR = os.path.join(BASE_DIR, f"checkpointazi_{TRAIN_START_TIME}")
LOG_DIR = os.path.join(BASE_DIR, f"logazi_{TRAIN_START_TIME}")
        ...
        with h5py.File(h5_file_path, "r") as f:
            ...
            for key in tqdm(valid_sample_keys, desc="加载H5三输入数据"):
                try:
                    ...
                    baz_deg = float(wave_group.attrs.get("azi", np.nan))
                    # dis = float(wave_group.attrs.get("dis", np.nan))
                    # if np.isnan(baz_deg) or dis > 200:
                    #     # logging.warning(f"样本{key}方位角无效，跳过")
                    #     continue
                    if np.isnan(baz_deg):
                        # logging.warning(f"样本{key}方位角无效，跳过")
                        continue
                    baz_rad = np.deg2rad(baz_deg)
                    label = np.array([np.sin(baz_rad), np.cos(baz_rad)], dtype=np.float32)
...
def train_one_epoch(model, loader, criterion, optimizer, scheduler, backbone_params, head_params, epoch):
    ...
    for i, (wave, spec, feat, y, _) in enumerate(tqdm(loader, desc=f"训练 Epoch {epoch}")):
        wave, spec, feat, y = wave.to(device), spec.to(device), feat.to(device), y.to(device)
        optimizer.zero_grad()
        ...
        if (i + 1) % 100 == 0:
            batch_ang_err = baz_angle_error(
                np.array(preds[-len(y):]), np.array(labels[-len(y):])
            )
            logging.debug(
                f"Epoch {epoch} 批次 {i + 1}/{len(loader)} - 批次损失: {loss.item():.6f} | 批次角度误差: {batch_ang_err:.3f}°")
    epoch_loss = total_loss / len(loader)
    epoch_ang_err = baz_angle_error(np.array(preds), np.array(labels))
    logging.debug(f"Epoch {epoch} 训练结束 - 平均损失: {epoch_loss:.6f} | 平均角度误差: {epoch_ang_err:.3f}°")
    return epoch_loss, epoch_ang_err
...
def evaluate(model, loader, criterion, mode="验证"):
    ...
    avg_ang_err = baz_angle_error(np.array(preds), np.array(labels))
    logging.info(f"{mode}结束 - 平均损失: {avg_loss:.6f} | 平均角度误差: {avg_ang_err:.3f}°")
    return avg_loss, avg_ang_err
...
def test_model(model, loader):
    ...
    with torch.no_grad():
        for wave, spec, feat, y, k in tqdm(loader, desc="测试中"):
            ...
            keys.extend(k)
    ang_err = baz_angle_error(np.array(preds), np.array(trues))
    logging.info(f"测试结果汇总:")
    logging.info(f" Mean Angular Error: {ang_err:.3f}°")
    return np.array(trues), np.array(preds), keys, (ang_err)
...
# -------------------------- 5. 主函数 --------------------------
def main(model_name="EEW_LMChronos_t5_azi", h5_path=None):
    set_deterministic(42)  # <-- 添加这一行
    logging.info("开始加载三输入 H5 数据集（BAZ 任务）...")
    ...
    # 最佳模型保存相关参数
    best_metrics = {
        "val_ang_err": float("inf"),
        "val_loss": float("inf"),
        "epoch": 0,
        "model_state": None
    }
    ckpt_path = os.path.join(
        CHECKPOINT_DIR, f"best_baz_model_{TRAIN_START_TIME}.pth"
    )
    early_stop_counter = 0
    train_losses, val_losses, val_ang_errs = [], [], []

    logging.info("开始 BAZ 训练...")
    for epoch in range(1, EPOCHS + 1):
        logging.info(f"\n========== Epoch {epoch}/{EPOCHS} ==========")
        logging.info(f"当前学习率: Backbone={optimizer.param_groups[0]['lr']:.6e}, Head={optimizer.param_groups[1]['lr']:.6e}")

        train_loss, train_ang_err = train_one_epoch(
            model, train_loader, criterion, optimizer, scheduler,
            backbone_params, head_params, epoch
        )
        val_loss, val_ang_err = evaluate(model, val_loader, criterion)

        train_losses.append(train_loss)
        val_losses.append(val_loss)
        val_ang_errs.append(val_ang_err)

        logging.info(
            f"Train Loss: {train_loss:.6f} | Train AngErr: {train_ang_err:.3f}° | "
            f"Val Loss: {val_loss:.6f} | Val AngErr: {val_ang_err:.3f}°"
        )

        # ---------- Early Stop ----------
        if val_ang_err < best_metrics["val_ang_err"]:
            best_metrics.update({
    ...
    preds, trues = [], []
    with torch.no_grad():
        for wave, spec, feat, y, _ in test_loader:
    ...
    test_ang_err = baz_angle_error(preds, trues)
    logging.info(f"测试集 Mean Angular Error: {test_ang_err:.3f}°")

    # ---------- Polar / Rose Diagram ----------
    true_deg = sincos_to_deg(trues[:, 0], trues[:, 1])
    pred_deg = sincos_to_deg(preds[:, 0], preds[:, 1])
    rose_path = os.path.join(CHECKPOINT_DIR, "baz_error_rose.png")
    plot_baz_polar(true_deg, pred_deg, rose_path)
    ...
    csv_path = os.path.join(CHECKPOINT_DIR, f"baz_test_results_{TRAIN_START_TIME}.csv")
    results_df.to_csv(csv_path, index=False)
    logging.info(f"测试结果已保存到 CSV: {csv_path}")
    logging.info("BAZ 训练与测试全部完成！")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train Seismic BAZ Estimation Model with configurable backbone")
    parser.add_argument("--model_name", type=str, default="EEW_LMChronos_t5_azi",
