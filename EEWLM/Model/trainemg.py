...
import csv
from transformers import get_linear_schedule_with_warmup  # 确保已导入
...
# 导入模型（请确保你的模型文件正确导入）
from inputhead  import EEW_LLMBART_mag
# from model_list import EEW_LLMChronos_t5_mag, EEW_LLM_t5_mag, EEW_LLM_GPT2_mag, EEW_LLMBERT_mag, EEW_LLMBART_mag
# from modelA import EEW_LLMBART_magA
# from modelAC import EEW_LLMBART_magAC
# from modelAB import EEW_LLMBART_magAB

MODEL_REGISTRY = {
    # "EEW_LLMChronos_t5_mag": EEW_LLMChronos_t5_mag,
    # "EEW_LLM_t5_mag": EEW_LLM_t5_mag,
    # "EEW_LLM_GPT2_mag": EEW_LLM_GPT2_mag,
    # "EEW_LLMBERT_mag": EEW_LLMBERT_mag,
    "EEW_LLMBART_mag": EEW_LLMBART_mag,
    # "EEW_LLMBART_magAB":EEW_LLMBART_magAB,
    # "EEW_LLMBART_magA":EEW_LLMBART_magA,
    # "EEW_LLMBART_magAC":EEW_LLMBART_magAC,
}
...
CHECKPOINT_DIR = os.path.join(BASE_DIR, f"checkpointemg_{TRAIN_START_TIME}")
LOG_DIR = os.path.join(BASE_DIR, f"logemg_{TRAIN_START_TIME}")
os.makedirs(CHECKPOINT_DIR, exist_ok=True)
os.makedirs(LOG_DIR, exist_ok=True)

# 数据与训练参数 - 适配H5三输入数据集
H5_FILE_PATH = r"G:\迁移学习\LLM\20251209NewTry\2636dataset\JKnet_300_Regression_FS.h5"  # 替换为你的H5文件路径
TARGET_LENGTH = 300
SPEC_LENGTH = 128
        ...
        with h5py.File(h5_file_path, "r") as f:
            ...
            for key in tqdm(valid_sample_keys, desc="加载H5三输入数据"):
                try:
                    ...
                    # 4. 加载标签（mag/dis/azi，这里默认mag）
                    # label = float(wave_group.attrs.get("mag", np.nan))
                    # dis = float(wave_group.attrs.get("dis", np.nan))
                    # if np.isnan(label) or label <= 0 or dis > 200:
                    #     # logging.warning(f"样本{key}标签无效，跳过")
                    #     continue
                    label = float(wave_group.attrs.get("mag", np.nan))
                    # dis = float(wave_group.attrs.get("dis", np.nan))
...
def train_one_epoch(model, loader, criterion, optimizer, scheduler, backbone_params, head_params, epoch):
    ...
    for i, (wave, spec, feat, y, _) in enumerate(tqdm(loader, desc=f"训练 Epoch {epoch}")):
        ...
        # 2. 前向传播（适配模型的wave, spec, feat三输入）
        optimizer.zero_grad()
        out = model(wave, spec, feat)  # 核心修改：传入三个输入参数
...
# -------------------------- 6. 主函数（保持框架不变，适配三输入数据）--------------------------
def main(model_name="EEW_LLMChronos_t5_mag", h5_path=None):
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        # LoRA 参数（来自 PEFT）通常包含 "lora" 字样
        if "lora" in name:
            backbone_params.append(param)
        else:
            # 所有非 LoRA 可训练参数视为 head
    ...
    scheduler = get_cosine_schedule_with_warmup(
        ...
        num_warmup_steps=num_warmup_steps,
        num_training_steps=total_steps,
        num_cycles=0.5,  # 关键：0.5 表示从 max_lr 平滑降到 0（标准余弦退火）
    )
    # 最佳模型保存相关参数
    best_metrics = {
        "val_mae": float("inf"),
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Train Seismic Magnitude Estimation Model with configurable backbone")
    parser.add_argument("--model_name", type=str, default="EEW_LLMBART_mag",
                        choices=list(MODEL_REGISTRY.keys()),
                        help=f"Model to train. Options: {list(MODEL_REGISTRY.keys())}")
    parser.add_argument("--h5_path", type=str, default=r"G:\迁移学习\LLM\20251209NewTry\2636dataset\JKnet_300_Regression_FS.h5", help="Path to the H5 dataset file")
